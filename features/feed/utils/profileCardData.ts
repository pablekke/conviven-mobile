...
export function toProfileCardSnapshot(profile: ProfileLike): ProfileCardSnapshot {
  ...
  const budgetLabel = `$${toInt(profile.filters?.budgetMin)}–$${toInt(profile.filters?.budgetMax)}`;
...
export function useProfileCardSnapshot(profile: ProfileLike): ProfileCardSnapshot {
  return useMemo(() => toProfileCardSnapshot(profile), [profile]);
}
